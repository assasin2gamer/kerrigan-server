...
void updateWindowTitle(GtkWindow* window, const AppData& app) {
    std::string baseTitle = "Order Book (MBO) Monitor";
...
int main(int argc, char *argv[])
{
    ...
    app.processor    = std::make_shared<DataProcessor>(app.dbClient);
    ...
    // Pre-populate tickerMap with all possible tickers from config, but toggled off by default
    for(const auto& sym : app.config.symbols) {
        // Only add if user toggles it in UI
    }

    GtkWidget *window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_default_size(GTK_WINDOW(window), 1200, 700);
    gtk_window_set_title(GTK_WINDOW(window), "Advanced MBO Terminal");
    g_signal_connect(window, "destroy", G_CALLBACK(gtk_main_quit), NULL);

    GtkWidget *mainBox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
    gtk_container_add(GTK_CONTAINER(window), mainBox);

    // Left side: Ticker toggles + Start/Stop
    ...
    // Right side: Notebook with Graph + Debug
